...
which converge like :math:`\sim 1/\sqrt{\frac{n_{samples}}{\tau_{inf}}}`. The
"effective" number of independent samples thus basically reduced by a factor
of :math:`\tau_{inf}`. (Note that depending on the definition of :math:`\tau_{inf}`, there may be an extra factor of 2, but this is already accounted for in our implemenetation.)
